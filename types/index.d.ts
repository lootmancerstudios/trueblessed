...
export interface IBlessedProgramOptions {
    ...
    /**
     * Enable or disable 24-bit RGB (truecolor) support.
     * When enabled, hex color tags like {#FF5733-fg} will render
     * as true 24-bit RGB colors instead of converting to 256-color palette.
     *
     * Auto-detected from:
     * - COLORTERM=truecolor or COLORTERM=24bit
     * - iTerm2 or VTE-based terminals
     *
     * @default auto-detected
     * @example
     * ```typescript
     * const screen = blessed.screen({
     *   truecolor: true  // Force enable
     * });
     * ```
     */
...
export class BlessedProgram extends EventEmitter {
    ...
    /**
     * Whether the terminal supports 24-bit RGB colors (truecolor).
     *
     * When true:
     * - Hex color tags render as true 24-bit RGB
     * - Screen can allocate RGB colors with IDs 256-510
     * - ANSI codes use `38;2;R;G;B` format instead of `38;5;N`
     *
     * When false:
     * - Hex colors are converted to nearest 256-color palette match
     * - Falls back to traditional color handling
     *
     * @readonly Auto-detected during construction
     */
...
export namespace Widgets {
    ...
    class Screen extends NodeWithEvents implements IHasOptions<IScreenOptions> {
        ...
        /**
         * Internal RGB color allocation map.
         * Maps color IDs (256-510) to RGB values { r, g, b }.
         *
         * Used by blessed's color rendering pipeline to:
         * 1. Allocate unique IDs for each RGB color
         * 2. Reuse IDs for duplicate RGB values
         * 3. Render true 24-bit ANSI codes during screen drawing
         *
         * @internal
         * @example
         * ```typescript
         * screen._rgbColors[256] // { r: 237, g: 163, b: 206 }
         * screen._rgbColors[257] // { r: 235, g: 167, b: 207 }
         * ```
         */
        _rgbColors: { [id: number]: { r: number; g: number; b: number } };

        /**
         * Current RGB color ID counter (256-510).
         * Used internally to allocate sequential IDs for new RGB colors.
         *
         * @internal
         */
        _rgbColorId: number;
         ...
         *
         * Process:
         * 1. Checks if RGB color already exists in _rgbColors
         * 2. If exists, returns existing ID
         * 3. If new, allocates next available ID (256-510)
         * 4. Stores RGB values in _rgbColors map
         *
         * @param r - Red component (0-255)
         * @param g - Green component (0-255)
         * @param b - Blue component (0-255)
         * @returns Color ID between 256-510, or existing ID if color already allocated
         *
         * @remarks
         * Only functional when `program.hasTruecolor` is true.
         * Falls back to 256-color matching when false.
         *
         * @internal Used by blessed's color parsing pipeline
         *
         * @example
         * ```typescript
         * // First allocation
         * const id1 = screen._allocRgbColor(255, 87, 51); // Returns 256
         *
         * // Reuse existing
         * const id2 = screen._allocRgbColor(255, 87, 51); // Returns 256 (same)
         *
         * // New color
         * const id3 = screen._allocRgbColor(100, 200, 150); // Returns 257
         * ```
         */
        _allocRgbColor(r: number, g: number, b: number): number;
